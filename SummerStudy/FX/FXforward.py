...
class FXF():
  ...
  def PRICING(self, usd_curve, krw_Curve, fx_spot):
    ...
    fxf = qe.FxForward(self.krw_notional, self.krw, self.usd_notional, self.usd, self.maturity_date, self.payCcy1)

    # FX Forward Pricing Engine
    engine = qe.DiscountingFxForwardEngine(self.krw, krw_curve_handle, self.usd, usd_curve_handle, fx_spot_handle)
  ...
  def KRW_IR_DELTA(self):
    ...
    # FX Forward price when 1bp up
    up_curve = qe.ZeroSpreadedTermStructure(curve_handle, qe.QuoteHandle(qe.SimpleQuote(basis_point)))
    up_fxf = self.PRICING(up_curve, self.usd_curve, self.fx_spot)

    # FX Forward price when 1bp donw
    down_curve = qe.ZeroSpreadedTermStructure(curve_handle, qe.QuoteHandle(qe.SimpleQuote(-basis_point)))
    down_fxf = self.PRICING(up_curve, self.usd_curve, self.fx_spot)
...
if __name__ == '__main__':
  ...
  # Build FXF object
  fxf = FXF(todays_date, maturity_date, fx_spot, fx_forward, usd_notional, position)
